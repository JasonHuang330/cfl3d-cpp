...
//
// extra.cpp — faithful C++ translation of extra.F (CFL3D ronnie).
// Compute extra mid-cell point (x5,y5,z5) at (xie,eta)=(.5,0.) via a
// quadratic least-squares extrapolation along the j (xie) direction.
//
           ...
           double& x5,double& y5,double& z5,int& icase,int ifit)
{
    ...
    y5 = 0.5*( y1 + y2 );
    z5 = 0.5*( z1 + z2 );
    if (jcell < jcl || jcell > jcr) goto L1500;
    if (jcl == jcr) goto L1500;
    dxp = x2 - x1;
    dyp = y2 - y1;
    dzp = z2 - z1;
    xlen = std::sqrt( dxp*dxp + dyp*dyp + dzp*dzp );
    if (xlen <= 0.0) goto L1500;
    if (jcell == jcl) goto L1000;
    if (jcell == jcr) goto L2000;
    ...
    d2_ = Z(jcell-1,kcell,l) - z1 - (z2-z1)*xlens;
    a2 = xlens*( 1.0 - xlens );
    x5 = x5 + 0.25*b2/a2;
    y5 = y5 + 0.25*c2/a2;
    z5 = z5 + 0.25*d2_/a2;
    ...
    d1_ = Z(jcell+2,kcell,l) - z1 - (z2-z1)*xlens;
    a1 = xlens*( 1.0 - xlens );
    ...
    z5 = 0.5*( z1 + z2 ) + term*( d1_*a1 + d2_*a2*trat );
    icase = 0;
    goto L1500;
