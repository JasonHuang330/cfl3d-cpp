...
|------|------|-----------------|
| **splitter** | block / input-file splitter — splits blocks and makes them multigridable | `main.txt` + split grid `grdflat5.bin` **byte-exact** vs Fortran |
...
```sh
bash tools/splitter/build_verify.sh
#   compiles, runs the bundled case, byte-compares -> "RESULT: BYTE-EXACT MATCH (stdout + split grid)"
...
| 工具 | 作用 | 已验证输出 |
|------|------|-----------|
| **splitter** | 分块 / 输入文件分割器 —— 拆分网格块并使其可多重网格化 | `main.txt` + 分割后网格 `grdflat5.bin` 与 Fortran **字节完全一致** |
| **ronnie** | 通用拼接网格（chimera）连通性预处理器 | `patch.bin` + `ronnie.out` 与 Fortran **字节完全一致** |
| **maggie** | 重叠网格挖洞 / 插值模板预处理器 | `ovrlp.bin` 与 Fortran **字节完全一致** |

```sh
bash tools/splitter/build_verify.sh
#   编译、跑算例、字节对比 -> "RESULT: BYTE-EXACT MATCH (stdout + split grid)"
...
| 도구 | 역할 | 검증된 출력 |
|------|------|------------|
| **splitter** | 블록 / 입력 파일 분할기 — 블록을 분할하고 다중 격자화 가능하게 만듦 | `main.txt` + 분할 격자 `grdflat5.bin` 이 Fortran 과 **바이트 단위 완전 일치** |
...
```sh
bash tools/splitter/build_verify.sh
#   컴파일·케이스 실행·바이트 비교 -> "RESULT: BYTE-EXACT MATCH (stdout + split grid)"
