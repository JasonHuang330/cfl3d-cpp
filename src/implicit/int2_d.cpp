...
void int2_d(int& iq, int& jdim1, int& kdim1, int& idim1, int& jmax2, int& kmax2, int& nsub1, int& jmax1, int& kmax1, int& l1, int& i1, FortranArray3DRef<double> q1, FortranArray3DRef<double> bc2, int& i2, FortranArray4DRef<double> q2, FortranArray1DRef<double> xie2, FortranArray1DRef<double> eta2, FortranArray2DRef<double> q1g, FortranArray2DRef<double> q2g, FortranArray1DRef<double> temp, FortranArray1DRef<int> nblkpt, int& intmax, int& icheck, int& mtype, FortranArray2DRef<int> iindex, int& ifo, int& ldim, int& npt, int& j21, int& j22, int& k21, int& k22, FortranArray3DRef<double> q1wk, double& dthtx, double& dthty, double& dthtz, int& lim_ptch)
{
    ...
    int nv, l, izz, i22, jd, kd, jsta, jend, ksta, kend;
    int ll, lc, jc, kc, j, k;
    ...
    if (mtype == 1) {
        ...
        if (iq > 0) {
            ...
            if (adthtx > 0. || adthty > 0. || adthtz > 0.) {
                ...
                ksta = 1;
                kend = kdim1;
                rotateq2_d_ns::rotateq2_d(jd, kd, q1, q1wk, jsta, jend,
        ...
        for (l = 1; l <= ldim; l++) {
            ...
            for (k = k21; k <= k22 - 1; k++) {
                for (j = j21; j <= j22 - 1; j++) {
                    ...
                    {
                        double one_r = 1.0;
                        double jmax1_r = (double)jmax1;
                        double kmax1_r = (double)kmax1;
                        xie2(ll) = ccomplex_ns::ccmaxcr(xie2(ll), one_r);
                        eta2(ll) = ccomplex_ns::ccmaxcr(eta2(ll), one_r);
                        xie2(ll) = ccomplex_ns::ccmincr(xie2(ll), jmax1_r);
                        eta2(ll) = ccomplex_ns::ccmincr(eta2(ll), kmax1_r);
                    }
...
{
    // Extract 3D slice at i1 from 4D arrays q1 and q1wk
    FortranArray3DRef<double> q1_3d(&q1(1, 1, i1, 1), jmax1, kmax1, ldim);
    FortranArray3DRef<double> q1wk_3d(&q1wk(1, 1, i1, 1), jmax1, kmax1, ldim);

    int2_d(iq, jdim1, kdim1, idim1, jmax2, kmax2, nsub1,
           jmax1, kmax1, l1, i1, q1_3d, bc2, i2, q2,
           xie2, eta2, q1g, q2g, temp, nblkpt, intmax, icheck, mtype,
           iindex, ifo, ldim, npt, j21, j22, k21, k22,
           q1wk_3d, dthtx, dthty, dthtz, lim_ptch);
}
