...
{
    bc2005_ns::bc(ntime, nbl, lw, lw2, w, mgwk, wk, nwork, cl,
...
void bc2005(int& jdim, int& kdim, int& idim,
            ...
            FortranArray1DRef<int> mblk2nd, int& maxbl, int& nummem)
{
    bc2005_ns::bc2005(jdim, kdim, idim, q, qj0, qk0, qi0,
                      ...
                      qrotj, qrotk, qroti,
                      nbl, nblp, nou, bou, nbuf, ibufdim,
                      myid, mblk2nd, maxbl, nummem);
}


// ============================================================
// bc2005k_d() — periodic BC for k=1 (nface=5) and k=kdim (nface=6)
// ============================================================
void bc2005k_d(int& jdim, int& kdim, int& idim,
               FortranArray4DRef<double> datak0,
               int& ista, int& iend, int& jsta, int& jend,
               int& ksta, int& kend, int& nface,
               int& mdim, int& ndim, FortranArray4DRef<double> bcdata,
               char (&filname)[80],
               int& jdimp, int& kdimp, int& idimp,
               FortranArray4DRef<double> datapk,
               int& nbl, int& nblp, int& ldim,
               FortranArray1DRef<int> nou, FortranArray2DRef<char[120]> bou,
               int& nbuf, int& ibufdim, int& myid,
               FortranArray1DRef<int> mblk2nd, int& maxbl, int& iflag)
{
    // COMMON block aliases
    int32_t& level    = cmn_mgrd.level;
    int32_t& lglobal  = cmn_mgrd.lglobal;
    int32_t* ivisc    = cmn_reyue.ivisc;   // 0-based: ivisc[0]=ivisc(1), etc.
    float&   radtodeg = cmn_conversion.radtodeg;
    int32_t& ialph    = cmn_igrdtyp.ialph;

    // Local rotation matrices (Fortran real = float), 1-based 3x3
    ...
    // This BC uses only one plane of bcdata
    int ip = 1;
    ...
    if (nface == 5) {
        ...
        if (iflag == 0) {
            // Rotate datapk values before setting datak0 (q data)
            int idim2 = 2;
            if (kdimp == 2) {
                int ip1 = 1, ip1e = 1;
                ...
                rotateq_ns::rotateq(jdimp, idim2, idimp, datapk, datapk,
                                    ip1, ip1e, ista, iend1, jsta, jend1,
                                    dthtx, dthty, dthtz);
                int ip2 = 2, ip2e = 2;
                rotateq_ns::rotateq(jdimp, idim2, idimp, datapk, datapk,
                                    ip2, ip2e, ista, iend1, jsta, jend1,
                                    dthtx2, dthty2, dthtz2);
            } else {
                int ip1 = 1, ip2 = 2;
                rotateq_ns::rotateq(jdimp, idim2, idimp, datapk, datapk,
                                    ip1, ip2, ista, iend1, jsta, jend1,
                                    dthtx, dthty, dthtz);
            }
                ...
                if (ivisc[2] >= 70 || ivisc[1] >= 70 || ivisc[0] >= 70) {
                    ...
                    for (int i = ista; i <= iend1; i++) {
                        for (int j = jsta; j <= jend1; j++) {
                            ...
                            for (int ll = 1; ll <= 7; ll++) tmp_in[ll-1] = (float)datapk(j,1,i,ll);
                            ...
                            for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,1) = (double)tmp_out[ll-1];
                            if (kdimp == 2) {
                                float tmp_in2[7], tmp_out2[7];
                                for (int ll = 1; ll <= 7; ll++) tmp_in2[ll-1] = (float)datak0(j,i,ll,1);
                                ...
                                for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,2) = (double)tmp_out2[ll-1];
                            } else {
                                float tmp_in3[7], tmp_out3[7];
                                for (int ll = 1; ll <= 7; ll++) tmp_in3[ll-1] = (float)datapk(j,2,i,ll);
                                ...
                                for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,2) = (double)tmp_out3[ll-1];
                            }
                        }
                    }
                ...
                if (ivisc[2] >= 70 || ivisc[1] >= 70 || ivisc[0] >= 70) {
                    ...
                    for (int i = ista; i <= iend1; i++) {
                        for (int j = jsta; j <= jend1; j++) {
                            ...
                            FortranArray1DRef<float> out_ref(tmp_out, 7);
                            module_stm_2005_ns::stm2k5_bc(in_ref, rn_ref, rnt_ref, out_ref);
                            for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,3) = (double)tmp_out[ll-1];
                            if (kdimp == 2) {
                                float tmp_in2[7], tmp_out2[7];
                                for (int ll = 1; ll <= 7; ll++) tmp_in2[ll-1] = (float)datak0(j,i,ll,3);
                                FortranArray1DRef<float> in2_ref(tmp_in2, 7);
                                FortranArray1DRef<float> out2_ref(tmp_out2, 7);
                                module_stm_2005_ns::stm2k5_bc(in2_ref, rn_ref, rnt_ref, out2_ref);
                                for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,4) = (double)tmp_out2[ll-1];
                            } else {
                                float tmp_in3[7], tmp_out3[7];
                                for (int ll = 1; ll <= 7; ll++) tmp_in3[ll-1] = (float)datapk(j,2,i,ll);
                                FortranArray1DRef<float> in3_ref(tmp_in3, 7);
                                FortranArray1DRef<float> out3_ref(tmp_out3, 7);
                                module_stm_2005_ns::stm2k5_bc(in3_ref, rn_ref, rnt_ref, out3_ref);
                                for (int ll = 1; ll <= 7; ll++) datak0(j,i,ll,4) = (double)tmp_out3[ll-1];
                            }
                        }
                    }
                } else if (ivisc[2] >= 4 || ivisc[1] >= 4 || ivisc[0] >= 4) {
