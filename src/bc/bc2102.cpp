...
void bc2102(int& jdim, int& kdim, int& idim,
            ...
            int& myid, int& nummem, int& iflag)
{
    ...
    // **************************************************************************
    //  j=1 boundary         set p(time), extrapolate others         bctype 2102
    // **************************************************************************
    ...
    if (nface == 3) {
        ...
        for (int i = ista; i <= iend1; i++) {
            ...
            for (int k = ksta; k <= kend1; k++) {
                int kk = k - ksta + 1;
                double pratio = bcdata(kk,ii,ip,1);
                double deltap = bcdata(kk,ii,ip,2);
                double rfreqp = bcdata(kk,ii,ip,3);
                double lref   = bcdata(kk,ii,ip,4);
                double phioff;
                if (iflag == 0) {
                    phioff = 0.0;
                } else {
                ...
                double pset = (pratio + deltap * sin(twopi * rfreqp / lref * (double)time + phioff)) * (double)p0;
                qj0(k,i,5,1) = pset;
                qj0(k,i,5,2) = pset;
            }
        }
    ...
    // **************************************************************************
    //  j=jdim boundary         set p(time), extrapolate others      bctype 2102
    // **************************************************************************
    ...
    // **************************************************************************
    //  k=1 boundary         set p(time), extrapolate others         bctype 2102
    // **************************************************************************
    ...
    if (nface == 5) {
        ...
        for (int i = ista; i <= iend1; i++) {
            ...
            for (int j = jsta; j <= jend1; j++) {
                int jj = j - jsta + 1;
                double pratio = bcdata(jj,ii,ip,1);
                double deltap = bcdata(jj,ii,ip,2);
                double rfreqp = bcdata(jj,ii,ip,3);
                double lref   = bcdata(jj,ii,ip,4);
                double phioff;
                if (iflag == 0) {
                    phioff = 0.0;
                } else {
    ...
    // **************************************************************************
    //  k=kdim boundary         set p(time), extrapolate others      bctype 2102
    // **************************************************************************
    ...
    // **************************************************************************
    //  i=1 boundary         set p(time), extrapolate others         bctype 2102
    // **************************************************************************
    ...
    if (nface == 1) {
        ...
        for (int k = ksta; k <= kend1; k++) {
            ...
            for (int j = jsta; j <= jend1; j++) {
                int jj = j - jsta + 1;
                double pratio = bcdata(jj,kk,ip,1);
                double deltap = bcdata(jj,kk,ip,2);
                double rfreqp = bcdata(jj,kk,ip,3);
                double lref   = bcdata(jj,kk,ip,4);
                double phioff;
                if (iflag == 0) {
                    phioff = 0.0;
                } else {
    ...
    // **************************************************************************
    //  i=idim boundary         set p(time), extrapolate others      bctype 2102
    // **************************************************************************
