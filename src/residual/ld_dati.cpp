...
void ld_dati(FortranArray4DRef<double> data, int& jdim, int& kdim, int& idim,
             FortranArray4DRef<double> datai, int& ldim,
             ...
             int& nbuf, int& ibufdim, int& myid)
{
    if (np > 4) {
        ...
        std::snprintf(bou(nou(1), 1), 120,
            "stopping...increase dimension of mp array in ld_dati and its calling routine");
        int ierrflg = -1;
        termn8_ns::termn8(myid, ierrflg, ibufdim, nbuf, bou, nou);
    }

    // load data values into datai array:
