...
// Helper: write Fortran unformatted record
...
void plot3d(int& jdim, int& kdim, int& idim, int& i1, int& i2, int& i3, int& j1, int& j2, int& j3, int& k1, int& k2, int& k3, FortranArray4DRef<double> q, FortranArray4DRef<double> qi0, FortranArray4DRef<double> qj0, FortranArray4DRef<double> qk0, FortranArray3DRef<double> x, FortranArray3DRef<double> y, FortranArray3DRef<double> z, FortranArray4DRef<double> xw, FortranArray4DRef<double> blank2, FortranArray3DRef<double> blank, FortranArray4DRef<double> xg, int& iflag, FortranArray3DRef<double> vist3d, int& iover, int& nblk, FortranArray1DRef<int> nmap, FortranArray3DRef<double> bcj, FortranArray3DRef<double> bck, FortranArray3DRef<double> bci, FortranArray4DRef<double> vj0, FortranArray4DRef<double> vk0, FortranArray4DRef<double> vi0, int& ifunc, int& iplot, int& jdw, int& kdw, int& idw, int& nplots, FortranArray1DRef<int> jdimg, FortranArray1DRef<int> kdimg, FortranArray1DRef<int> idimg, FortranArray1DRef<int> nblcg, FortranArray1DRef<int> jsg, FortranArray1DRef<int> ksg, FortranArray1DRef<int> isg, FortranArray1DRef<int> jeg, FortranArray1DRef<int> keg, FortranArray1DRef<int> ieg, int& ninter, FortranArray2DRef<int> iindex, int& intmax, int& nsub1, int& maxxe, FortranArray2DRef<int> nblkk, int& nbli, FortranArray3DRef<int> limblk, FortranArray3DRef<int> isva, FortranArray1DRef<int> nblon, int& mxbli, FortranArray1DRef<double> thetay, int& maxbl, int& maxgr, int& myid, int& myhost, int& mycomm, FortranArray1DRef<int> mblk2nd, FortranArray2DRef<int> inpl3d, int& nblock, FortranArray1DRef<int> nblkpt, FortranArray4DRef<double> xv, FortranArray4DRef<double> sj, FortranArray4DRef<double> sk, FortranArray4DRef<double> si, FortranArray3DRef<double> vol, int& nset)
{
    ...
    int jj, kk, ii;
    int nnn;
    int inter, lmax1, nbl, lst, lcoord, lend, j21, k21;
    int ll, mblk1, mblk2, mblk3, mblk4;
    int nblc, nblcc, js, ks, is, je, ke, ie;
    int it, ir, itime, iti;
    int is1, ie1, js1, je1, ks1, ke1;
    int ibembed;
    int ldw, leq;
    double cpc;
    int ix, jx, kx, im1, jm1, km1;
    double sim1, sim2, sim3, sjm1, sjm2, sjm3, skm1, skm2, skm3;
    double vnorm;
    // printout variables
    int icp, isay;
    double term1, term2, term3;
    double q1, q2, q3, q4, q5, t1, xm1, edvis, pitot;
    // viscous data variables
    ...
    // Allocatable local array
    FortranArray3D<double> xvist3d;
    ...
    if (iflag == 1 && iblnk > 0) {
        ...
        for (nnn = 1; nnn <= 2; nnn++) {
            for (j = j11; j <= j22; j++) {
                for (k = k11; k <= k22; k++) {
                    kk = std::min(k+1, k22);
                    jj = std::min(j+1, j22);
                    double a1 = bci(j,k,nnn);
                    double a2 = bci(jj,k,nnn);
                    double a3 = bci(j,kk,nnn);
                    double a4 = bci(jj,kk,nnn);
        ...
        for (nnn = 1; nnn <= 2; nnn++) {
            for (i = i11; i <= i22; i++) {
                for (k = k11; k <= k22; k++) {
                    kk = std::min(k+1, k22);
                    ii = std::min(i+1, i22);
                    double a1 = bcj(k,i,nnn);
                    double a2 = bcj(kk,i,nnn);
                    double a3 = bcj(k,ii,nnn);
                    double a4 = bcj(kk,ii,nnn);
                    blank2(j,k,i,1) = 1. + ccomplex_ns::ccmax4(a1, a2, a3, a4);
                }
            }
            j = jdim;
        ...
        for (nnn = 1; nnn <= 2; nnn++) {
            for (j = j11; j <= j22; j++) {
                for (i = i11; i <= i22; i++) {
                    jj = std::min(j+1, j22);
                    ii = std::min(i+1, i22);
                    double a1 = bck(j,i,nnn);
                    double a2 = bck(jj,i,nnn);
                    double a3 = bck(j,ii,nnn);
                    double a4 = bck(jj,ii,nnn);
                    blank2(j,k,i,1) = 1. + ccomplex_ns::ccmax4(a1, a2, a3, a4);
                }
            }
            k = kdim;
        ...
        if (iover == 1) {
            ...
            for (j = 2; j <= jdim1; j++) {
                for (k = 2; k <= kdim1; k++) {
                    double b1, b2, b3, b4;
                    b1 = blank(j,k,1);   b2 = blank(j-1,k,1);
                    b3 = blank(j,k-1,1); b4 = blank(j-1,k-1,1);
                    blank2(j,k,1,2)    = ccomplex_ns::ccmin4(b1, b2, b3, b4);
                    b1 = blank(j,k,idim1);   b2 = blank(j-1,k,idim1);
                    b3 = blank(j,k-1,idim1); b4 = blank(j-1,k-1,idim1);
            ...
            for (m = 1; m <= 2; m++) {
                ...
                for (j = 2; j <= jdim1; j++) {
                    double a1, a2;
                    a1 = blank(j,kk,1);   a2 = blank(j-1,kk,1);
                    blank2(j,k,1,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(j,kk,idim1); a2 = blank(j-1,kk,idim1);
                    blank2(j,k,idim,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            for (m = 1; m <= 2; m++) {
                j  = 1;
                ...
                for (k = 2; k <= kdim1; k++) {
                    double a1, a2;
                    a1 = blank(jj,k,1);   a2 = blank(jj,k-1,1);
                    blank2(j,k,1,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(jj,k,idim1); a2 = blank(jj,k-1,idim1);
                    blank2(j,k,idim,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            ...
            for (i = 2; i <= idim1; i++) {
                for (k = 2; k <= kdim1; k++) {
                    double b1, b2, b3, b4;
                    b1 = blank(1,k,i);   b2 = blank(1,k,i-1);
                    b3 = blank(1,k-1,i); b4 = blank(1,k-1,i-1);
                    blank2(1,k,i,2)    = ccomplex_ns::ccmin4(b1, b2, b3, b4);
                    b1 = blank(jdim1,k,i);   b2 = blank(jdim1,k,i-1);
                    b3 = blank(jdim1,k-1,i); b4 = blank(jdim1,k-1,i-1);
            ...
            for (m = 1; m <= 2; m++) {
                ...
                for (i = 2; i <= idim1; i++) {
                    double a1, a2;
                    a1 = blank(1,kk,i);   a2 = blank(1,kk,i-1);
                    blank2(1,k,i,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(jdim1,kk,i); a2 = blank(jdim1,kk,i-1);
                    blank2(jdim,k,i,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            for (m = 1; m <= 2; m++) {
                i  = 1;
                ...
                for (k = 2; k <= kdim1; k++) {
                    double a1, a2;
                    a1 = blank(1,k,ii);   a2 = blank(1,k-1,ii);
                    blank2(1,k,i,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(jdim1,k,ii); a2 = blank(jdim1,k-1,ii);
                    blank2(jdim,k,i,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            ...
            for (i = 2; i <= idim1; i++) {
                for (j = 2; j <= jdim1; j++) {
                    double b1, b2, b3, b4;
                    b1 = blank(j,1,i);   b2 = blank(j,1,i-1);
                    b3 = blank(j-1,1,i); b4 = blank(j-1,1,i-1);
                    blank2(j,1,i,2)    = ccomplex_ns::ccmin4(b1, b2, b3, b4);
                    b1 = blank(j,kdim1,i);   b2 = blank(j,kdim1,i-1);
                    b3 = blank(j-1,kdim1,i); b4 = blank(j-1,kdim1,i-1);
            ...
            for (m = 1; m <= 2; m++) {
                ...
                for (i = 2; i <= idim1; i++) {
                    double a1, a2;
                    a1 = blank(jj,1,i);   a2 = blank(jj,1,i-1);
                    blank2(j,1,i,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(jj,kdim1,i); a2 = blank(jj,kdim1,i-1);
                    blank2(j,kdim,i,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            for (m = 1; m <= 2; m++) {
                i  = 1;
                ...
                for (j = 2; j <= jdim1; j++) {
                    double a1, a2;
                    a1 = blank(j,1,ii);   a2 = blank(j-1,1,ii);
                    blank2(j,1,i,2)    = ccomplex_ns::ccmin(a1, a2);
                    a1 = blank(j,kdim1,ii); a2 = blank(j-1,kdim1,ii);
                    blank2(j,kdim,i,2) = ccomplex_ns::ccmin(a1, a2);
                }
            }
            ...
            for (i = 2; i <= idim1; i++) {
                for (j = 2; j <= jdim1; j++) {
                    for (k = 2; k <= kdim1; k++) {
                        double b1 = blank(j,k,i);
                        double b2 = blank(j-1,k,i);
                        double b3 = blank(j,k-1,i);
                        double b4 = blank(j-1,k-1,i);
                        double b5 = blank(j,k,i-1);
                        double b6 = blank(j-1,k,i-1);
                        double b7 = blank(j,k-1,i-1);
                        double b8 = blank(j-1,k-1,i-1);
                        blank2(j,k,i,2) = ccomplex_ns::ccmin8(b1,b2,b3,b4,b5,b6,b7,b8);
                    }
                }
            }
            ...
            for (i = 1; i <= idim; i++) {
                for (j = 1; j <= jdim; j++) {
                    for (k = 1; k <= kdim; k++) {
                        double a1 = blank2(j,k,i,1);
                        double a2 = blank2(j,k,i,2);
    ...
    if (iflag == 1) {
        ...
        if (lhdr > 0 && myid == myhost) {
            ...
            if (inpl3d(iplot,2) == -1) {
                fprintf(f11, "   plot3dg file is an xyz file at grid points\n");
                fprintf(f11, "   plot3dq file is a    q file at grid points with off-surface data used when output is on body only\n");
            }
            if (ifunc == -5) {
                fprintf(f11, "   plot3dg file is an xyz file at grid points\n");
            ...
            if (ifunc == -4) {
                fprintf(f11, "   plot3dg file is an xyz file at grid points\n");
                fprintf(f11, "   plot3dq file is a function file of amut at grid points\n");
            }
            if (i2d == 1) {
                if (iblnk == 0) {
        ...
        if (myid == myhost && icall1 == 0) {
            ...
            if (ibin == 0) {
                ...
                if (i2d == 0) {
                    if (iblnk == 0) {
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,1));
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,2));
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,3));
                        fprintf(f3,"\n");
                    } else {
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,1));
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,2));
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)xg(j,k,i,3));
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f3,"%14.6e",(float)(int)xg(j,k,i,4));
                        fprintf(f3,"\n");
                    }
                } else {
                    if (iblnk == 0) {
                ...
                if (i2d == 0) {
                    if (iblnk == 0) {
                        int32_t rb = (int32_t)(3*iw*jw*kw*(int)sizeof(float));
                        write_rec_begin(f3,rb);
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,1); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,2); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,3); fwrite(&v,sizeof(float),1,f3); }
                        write_rec_end(f3,rb);
                    } else {
                        int32_t rb = (int32_t)(3*iw*jw*kw*(int)sizeof(float)+iw*jw*kw*(int)sizeof(int32_t));
                        write_rec_begin(f3,rb);
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,1); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,2); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,3); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { int32_t v=(int32_t)xg(j,k,i,4); fwrite(&v,sizeof(int32_t),1,f3); }
                        write_rec_end(f3,rb);
                    }
                } else {
                    if (iblnk == 0) {
                        int32_t rb = (int32_t)(2*iw*jw*kw*(int)sizeof(float));
                        write_rec_begin(f3,rb);
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,1); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,3); fwrite(&v,sizeof(float),1,f3); }
                        write_rec_end(f3,rb);
                    } else {
                        int32_t rb = (int32_t)(2*iw*jw*kw*(int)sizeof(float)+iw*jw*kw*(int)sizeof(int32_t));
                        write_rec_begin(f3,rb);
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,1); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xg(j,k,i,3); fwrite(&v,sizeof(float),1,f3); }
                        for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { int32_t v=(int32_t)xg(j,k,i,4); fwrite(&v,sizeof(int32_t),1,f3); }
        ...
        if (ifunc == -4) {
            xvist3d.allocate(jdim, kdim, idim);
            {
                int jdwt=jdw, kdwt=kdw, idwt=idw, ldwt=1;
                // vist3d is 3D, pass as 4D with last dim=1
                FortranArray4DRef<double> vist3d_4d(vist3d.data(), jdim, kdim, idim, 1);
                FortranArray4DRef<double> xvist3d_4d(xvist3d.data(), jdw, kdw, idw, 1);
                cctogp_ns::cctogp(jdim,kdim,idim,i1,i2,i3,j1,j2,j3,k1,k2,k3,
                                  vist3d_4d,vi0,vj0,vk0,jdwt,kdwt,idwt,xvist3d_4d,ldwt);
            }
        } else {
            if (inpl3d(iplot,2) == -1) {
                if (i1 == i2 && i1 == 1) {
                    int i1t=i1+1, i2t=i2+1, i3t=i3, j1t=j1, j2t=j2, j3t=j3, k1t=k1, k2t=k2, k3t=k3;
                    cctogp_ns::cctogp(jdim,kdim,idim,i1t,i2t,i3t,j1t,j2t,j3t,k1t,k2t,k3t,
                ...
                if (j1 == j2 && j1 == 1) {
                    int i1t=i1, i2t=i2, i3t=i3, j1t=j1+1, j2t=j2+1, j3t=j3, k1t=k1, k2t=k2, k3t=k3;
                    cctogp_ns::cctogp(jdim,kdim,idim,i1t,i2t,i3t,j1t,j2t,j3t,k1t,k2t,k3t,
                                      q,qi0,qj0,qk0,jdw,kdw,idw,xv,ldw);
                }
                if (j1 == j2 && j1 == jdim) {
                ...
                if (k1 == k2 && k1 == 1) {
                    int i1t=i1, i2t=i2, i3t=i3, j1t=j1, j2t=j2, j3t=j3, k1t=k1+1, k2t=k2+1, k3t=k3;
                    cctogp_ns::cctogp(jdim,kdim,idim,i1t,i2t,i3t,j1t,j2t,j3t,k1t,k2t,k3t,
                                      q,qi0,qj0,qk0,jdw,kdw,idw,xv,ldw);
                }
                if (k1 == k2 && k1 == kdim) {
        ...
        if (ifunc == 0) {
            // special: if inpl3d(iplot,2) is -1, find approx wall-parallel components
            if (inpl3d(iplot,2) == -1) {
                if (i1 == i2) {
                    ix = 0;
                    for (i = i1; i <= i2; i += i3) {
            ...
            // load solution (q) in xv array - convert to conservative form
            for (i = 1; i <= iw; i++) {
                for (k = 1; k <= kw; k++) {
                    for (j = 1; j <= jw; j++) {
                        xv(j,k,i,5) = xv(j,k,i,5)/(double)gm1 + 0.5*(xv(j,k,i,2)*xv(j,k,i,2)
            ...
            if (ifunc == -5) {
                ...
                for (i = 1; i <= iw; i++) {
                    for (k = 1; k <= kw; k++) {
                        for (j = 1; j <= jw; j++) {
                            xv(j,k,i,1) = (xv(j,k,i,5)/(double)p0 - 1.)*cpc;
                        }
                    }
                }
            }
            if (ifunc == -6) {
                for (i = 1; i <= iw; i++) {
                    for (k = 1; k <= kw; k++) {
                        for (j = 1; j <= jw; j++) {
                            xv(j,k,i,1) = xv(j,k,i,5)/(double)p0;
                        }
                    }
                }
            }
        ...
        if (myid == myhost) {
            ...
            if (ifunc == 0) {
                if (ibin == 0) {
                    ...
                    if (i2d == 0) {
                        if (ialph == 0) {
                            for (m=1;m<=5;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,m));
                            fprintf(f4,"\n");
                        } else {
                            for (i=1;i<=iw;i++) for (j=1;j<=jw;j++) for (k=1;k<=kw;k++) xw(j,k,i,3) = -xw(j,k,i,3);
                            for (m=1;m<=2;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,m));
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,4));
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,3));
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,5));
                            fprintf(f4,"\n");
                        }
                    } else {
                        for (m=1;m<=2;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,m));
                        for (m=4;m<=5;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,m));
                        fprintf(f4,"\n");
                    }
                } else {
                    // binary
                    {
                        ...
                        write_rec_begin(f4,rb);
                        float v; v=(float)xmachw; fwrite(&v,sizeof(float),1,f4);
                        v=(float)alphww; fwrite(&v,sizeof(float),1,f4);
                        v=(float)reuew;  fwrite(&v,sizeof(float),1,f4);
                        v=(float)timew;  fwrite(&v,sizeof(float),1,f4);
                        write_rec_end(f4,rb);
                    }
                    if (i2d == 0) {
                        if (ialph == 0) {
                            int32_t rb = (int32_t)(5*iw*jw*kw*(int)sizeof(float));
                            write_rec_begin(f4,rb);
                            for (m=1;m<=5;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,m); fwrite(&v,sizeof(float),1,f4); }
                            write_rec_end(f4,rb);
                        } else {
                            for (i=1;i<=iw;i++) for (j=1;j<=jw;j++) for (k=1;k<=kw;k++) xw(j,k,i,3) = -xw(j,k,i,3);
                            int32_t rb = (int32_t)(5*iw*jw*kw*(int)sizeof(float));
                            write_rec_begin(f4,rb);
                            for (m=1;m<=2;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,m); fwrite(&v,sizeof(float),1,f4); }
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,4); fwrite(&v,sizeof(float),1,f4); }
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,3); fwrite(&v,sizeof(float),1,f4); }
                            for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,5); fwrite(&v,sizeof(float),1,f4); }
                            write_rec_end(f4,rb);
                        }
                    } else {
                        int32_t rb = (int32_t)(4*iw*jw*kw*(int)sizeof(float));
                        write_rec_begin(f4,rb);
                        for (m=1;m<=2;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,m); fwrite(&v,sizeof(float),1,f4); }
                        for (m=4;m<=5;m++) for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,m); fwrite(&v,sizeof(float),1,f4); }
                        write_rec_end(f4,rb);
                    }
                }
            } else {
                ...
                if (ibin == 0) {
                    for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) fprintf(f4,"%14.6e",(float)xw(j,k,i,1));
                    fprintf(f4,"\n");
                } else {
                    int32_t rb = (int32_t)(iw*jw*kw*(int)sizeof(float));
                    write_rec_begin(f4,rb);
                    for (k=1;k<=kw;k++) for (j=1;j<=jw;j++) for (i=1;i<=iw;i++) { float v=(float)xw(j,k,i,1); fwrite(&v,sizeof(float),1,f4); }
                    write_rec_end(f4,rb);
                }
            }
        } // myid == myhost
    ...
    if (iflag == 2) {
        ...
        if (ivisc[2] > 1 || ivisc[1] > 1 || ivisc[0] > 1) {
            ...
            {
                FortranArray4DRef<double> vist3d_4d(vist3d.data(), jdim, kdim, idim, 1);
                FortranArray4DRef<double> xv4_ref(xv.data(), jdw, kdw, idw, 5);
                // pass xv(1,1,1,4) as the output - offset by 3*jdw*kdw*idw elements
                // We use a view starting at element (1,1,1,4) of xv
                int offset = 3 * jdw * kdw * idw;
                FortranArray4DRef<double> xv4_slice(xv.data() + offset, jdw, kdw, idw, 1);
                int ldwt = 1;
                cctogp_ns::cctogp(jdim,kdim,idim,i1,i2,i3,j1,j2,j3,k1,k2,k3,
                                  vist3d_4d,vi0,vj0,vk0,jdw,kdw,idw,xv4_slice,ldwt);
            }
            for (i = 1; i <= idw; i++) {
                for (k = 1; k <= kdw; k++) {
                    for (j = 1; j <= jdw; j++) {
        ...
        if (myid == myhost) {
            ...
            term3 = 1./std::pow(1.+0.5*(double)gm1*(double)xmach*(double)xmach, (double)gamma/(double)gm1);
            iw = 0;
            for (i = i1; i <= i2; i += i3) {
                iw = iw + 1;
                kw = 0;
                for (k = k1; k <= k2; k += k3) {
                    ...
                    for (j = j1; j <= j2; j += j3) {
                        jw = jw + 1;
                        q1    = xw(jw,kw,iw,1);
                        q2    = xw(jw,kw,iw,2)/(double)xmach;
                        q3    = xw(jw,kw,iw,3)/(double)xmach;
                        q4    = xw(jw,kw,iw,4)/(double)xmach;
                        q5    = (double)gamma*xw(jw,kw,iw,5);
                        t1    = q5/q1;
                        xm1   = std::sqrt((double)xmach*(double)xmach*(q2*q2+q3*q3+q4*q4)/t1);
                        edvis = xg(jw,kw,iw,4);
                        if (icp == 1) {
                            pitot = 2.*(q5-1.)/((double)gamma*(double)xmach*(double)xmach);
                        } else {
                            if ((float)xm1 > 1.0f) {
                                term1 = std::pow(0.5*(double)gp1*xm1*xm1, (double)gamma/(double)gm1);
                                term2 = std::pow(2.*(double)gamma*xm1*xm1/(double)gp1 - (double)gm1/(double)gp1, 1./(double)gm1);
        ...
        if (ivisc[2] != 0) {
            ...
            for (i = i1; i <= i2; i += i3) {
                ...
                for (k = k1; k <= k2; k += k3) {
                    ...
                    for (j = j1; j <= j2; j += j3) {
                        ...
                        q5k1 = (double)gamma*.25*(qk0(j,id,5,m)  +qk0(j-1,id,5,m)
                        ...
                        q5k2 = (double)gamma*.25*(q(j,kd,id,5)   +q(j-1,kd,id,5)
                        ...
                        Cf = 2.0*(emuka+avgmut)/((double)reue*(double)xmach)*Cf/dn;
                        if ((float)q2k2 < 0.f) Cf = -Cf;
                        cfx  = 4.*(emuka+avgmut)/((double)reue*(double)xmach)*upar/dn;
                        cfy  = 4.*(emuka+avgmut)/((double)reue*(double)xmach)*vpar/dn;
                        cfz  = 4.*(emuka+avgmut)/((double)reue*(double)xmach)*wpar/dn;
                        eps  = 1.0e-6;
                        tty1 = 1.0 + (double)gm1*0.5*(double)xmach*(double)xmach;
                        t1m1 = t1k1-tty1;
                        if (std::abs((float)t1m1) <= (float)eps) {
                            Ch = 999.99999;
                        } else {
                            Ch = 2.*(t1k2-t1k1);
                            Ch = (emuka+avgmut)/((double)reue*(double)pr*(t1k1-tty1))*Ch/dn;
                        }
                        pres1 = q5k1;
                        temp1 = t1k1;
                        yplus = 0.;
                        if (std::abs((float)Cf) > 0.f) {
                            double cfabs = Cf;
                            yplus = dn*(double)reue*q1k1*std::sqrt(ccomplex_ns::ccabs(cfabs)*0.5/q1k1)/emuka;
                        }
                        xg(jw,kw,iw,1) = x(j,k,i);
                        xg(jw,kw,iw,2) = y(j,k,i);
                        xg(jw,kw,iw,3) = z(j,k,i);
            ...
            // write printout data to file (k surfaces)
            if (myid == myhost) {
                isay = 0;
                FILE* f17 = fortran_get_unit(17);
                iw = 0;
                        ...
                        for (j = j1; j <= j2; j += j3) {
                            ...
                            if (isay == 1)
                                fprintf(f17, "\n   I   J   K         X                Y                Z"
                                        ...
                                        "             Cfx             Cfy             Cfz\n");
                            if (ialph == 0) {
                                fprintf(f17, "%5d%5d%5d %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e\n",
                                        i,j,k,
        ...
        if (ivisc[1] != 0) {
            ...
            for (i = i1; i <= i2; i += i3) {
                ...
                for (j = j1; j <= j2; j += j3) {
                    ...
                    for (k = k1; k <= k2; k += k3) {
                        ...
                        q5j1 = (double)gamma*.25*(qj0(k,id,5,m)  +qj0(k-1,id,5,m)
                        ...
                        q5j2 = (double)gamma*.25*(q(jd,k,id,5)   +q(jd,k-1,id,5)
                        ...
                        if (std::abs((float)t1m1) <= (float)eps) {
                            ...
                            Ch = 2.*(t1j2-t1j1);
                            Ch = (emuka+avgmut)/((double)reue*(double)pr*(t1j1-tty1))*Ch/dn;
                        }
                        pres1 = q5j1;
                        temp1 = t1j1;
                        yplus = 0.;
                        if (std::abs((float)Cf) > 0.f) {
                            double cfabs = Cf;
                            yplus = dn*(double)reue*q1j1*std::sqrt(ccomplex_ns::ccabs(cfabs)*0.5/q1j1)/emuka;
                        }
                        xg(jw,kw,iw,1) = x(j,k,i);
                        xg(jw,kw,iw,2) = y(j,k,i);
                        xg(jw,kw,iw,3) = z(j,k,i);
            ...
            // write printout data to file (j surfaces)
            if (myid == myhost) {
                isay = 0;
                FILE* f17 = fortran_get_unit(17);
                iw = 0;
        ...
        if (ivisc[0] != 0) {
            ...
            for (j = j1; j <= j2; j += j3) {
                ...
                for (i = i1; i <= i2; i += i3) {
                    ...
                    for (k = k1; k <= k2; k += k3) {
                        ...
                        q5i1 = (double)gamma*.25*(qi0(j,k,5,m)   +qi0(j-1,k,5,m)
                        ...
                        q5i2 = (double)gamma*.25*(q(j,k,id,5)    +q(j,k-1,id,5)
                        ...
                        if (std::abs((float)t1m1) <= (float)eps) {
                            ...
                            Ch = 2.*(t1i2-t1i1);
                            Ch = (emuka+avgmut)/((double)reue*(double)pr*(t1i1-tty1))*Ch/dn;
                        }
                        pres1 = q5i1;
                        temp1 = t1i1;
                        yplus = 0.;
                        if (std::abs((float)Cf) > 0.f) {
                            double cfabs = Cf;
                            yplus = dn*(double)reue*q1i1*std::sqrt(ccomplex_ns::ccabs(cfabs)*0.5/q1i1)/emuka;
                        }
                        xg(jw,kw,iw,1) = x(j,k,i);
                        xg(jw,kw,iw,2) = y(j,k,i);
                        xg(jw,kw,iw,3) = z(j,k,i);
            ...
            // write printout data to file (i surfaces)
            if (myid == myhost) {
                isay = 0;
                FILE* f17 = fortran_get_unit(17);
                jw = 0;
